...
package loki // import "github.com/andresousafd/opentelemetry-collector-contrib/pkg/translator/loki"
...
func TestLogToLokiEntry(t *testing.T) {
	...
	for _, tt := range testCases {
		if tt.name == "with unknown format hint" {
			t.Run(tt.name, func(t *testing.T) {
				t.Skipf("skipping test '%v'. see https://github.com/andresousafd/opentelemetry-collector-contrib/issues/20240 for details.", tt.name)
			})
			continue
		}
